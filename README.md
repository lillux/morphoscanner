...
Other branches other than ***master*** can be installed adding *@branch_name* after the repository url:
```bash
...
**This version of Morphoscanner (0.0.1) support only simulation in which all the peptide have the same number
of aminoacids (same length).**\
...
- **Assisted Parsing**
If in your simulation you inserted premade aggregate, or groups of peptides as a single entities, you have to specify the size of a single peptide of the group. **To note that all the peptide of the simulation should be of the same aminoacids number (same length) in Morphoscanner 0.0.1.**\
...
After choosing the correct *peptide_length* and *interval*, put them inside the *compose_database* function:
``` python
