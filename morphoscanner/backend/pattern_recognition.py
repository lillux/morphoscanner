...
from .utilities import contact_list_from_dict, contact_list_from_dict_v1
...
def normalized_cross_correlation_function_torch_v1(contact_map, minimum_contact=2):
    '''
    Calculate normalized cross correlation function between a contact map and an ideal map.
...
def cross_correlation_function_for_dataset_with_dataframe_torch_v1(contact_array):
    '''
    Perform Normalized Cross Correlation function on the dataset
    to check for contact. Get a dict for processing and a pandas.DataFrame
    for data analysis
    ...
    for row in range(len(contact_array)):
        ...
        for col in range((row+1), len(contact_array[row])):
            ...
            best_match = normalized_cross_correlation_function_torch_v1(contact_array[row][col])
...
def denoise_contact_maps_torch_v1(contact_maps):
    ...
    '''
    normalized_cross_correlation_results, df, denoised_dict = cross_correlation_function_for_dataset_with_dataframe_torch_v1(contact_maps)
